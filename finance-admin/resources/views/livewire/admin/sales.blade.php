                                ...
                                <div class="table-container">
                                    <table class="table">
                                        ...
                                        @foreach ($sales as $item)
                                            <tbody wire:key="{{ $item->sales_code }}">
                                                <tr>
                                                    <td>{{ $item->sales_info }}</td>
                                                    <td>{{ $item->sales_product_name }}</td>
                                                    <td>{{ $item->sales_revenue }}</td>
                                                    <td>
                                                        @foreach ($departments as $department)
                                                            @if ($department->department_code == $item->sales_department)
                                                    ...
                                                    <td>
                                                        @foreach ($categories as $category)
                                                            @if ($category->plan_category_code == $item->sales_category)
                                                    ...
                                                    <td>
                                                        <ul class="action">
                                                            <li class="edit"> <a
                                                                    wire:click="edit({{ $item->sales_code }})"><i
                                                                        class="icon-pencil-alt"></i></a>
                                                            </li>
                                                            <li class="delete"><a
                                                                    wire:click="delete({{ $item->sales_code }})"><i
                        ...
                        <form wire:submit.prevent="{{ $salesId ? 'update' : 'store' }}">
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="row g-3">
                                        <div class="col-md-4">
                                            <x-input-text label="Info" name="sales_info" wire:model="sales_info" />
                                        </div>
                                        <div class="col-md-4">
                                            <x-input-text label="Name" name="sales_product_name" wire:model="sales_product_name" />
                                        </div>
                                        <div class="col-md-4">
                                            <x-select label="Department" name="sales_department" :options="$departments"
                                                valueId="department_code" valueName="department_name"
                                                wire:model="sales_department" />
                                        </div>
                                        <div class="col-md-4">
                                            <x-select label="Category" name="sales_category" :options="$categories"
                                                valueId="plan_category_code" valueName="plan_category_name" optvalue="category_code"
                                                wire:model="sales_category" />
                                        </div>
                                        <div class="col-md-4">
                                            <x-input-number label="Revenue" name="sales_revenue" wire:model="sales_revenue" />
                                        </div>
                                        <div class="col-md-4">
                                            <x-date label="Date" name="sales_date" wire:model="sales_date" />
                                        </div>
